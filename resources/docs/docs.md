...
The `home-routes` handler in the `border-patrol.routes.home` namespace
...
The `home-page` function will in turn call the `border-patrol.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `border-patrol.handler` namespace:
...
Request middleware functions are located under the `border-patrol.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `border-patrol.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `border-patrol.db.core` in some other namespace.
